...
export class AutoSuggest extends Component {
  ...
  setupListeners () {
    ...
    this.input.addEventListener(
      'blur',
      this.hideSuggestionsIfFocusedLost.bind(this)
    )
    this.container.addEventListener(
      'keydown',
      this.containerKeyDown.bind(this)
    )
  ...
  moveFocus (forward = true) {
    const focusables = Array.from(
      this.container.querySelectorAll('input,button')
    )
  ...
  displaySuggestions (suggestions) {
    ...
    this.list.innerHTML = suggestions
      .map(
        (value) =>
          `<li><button type="button" class="text-item">${escapeHtml(
            value
          )}</button></li>`
      )
      .join('')
