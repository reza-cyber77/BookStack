...
export class ImageManager extends Component {
  ...
  setupListeners () {
    ...
    this.searchInput.addEventListener('input', (event) => {
      this.cancelSearch.classList.toggle(
        'active',
    ...
    this.listContainer.addEventListener(
      'event-emit-select-image',
      this.onImageSelectEvent.bind(this)
    )
  ...
  async loadGallery () {
    ...
    const { data: html } = await window.$http.get(
      `images/${this.type}`,
      params
    )
    if (params.page === 1) {
  ...
  async loadImageEditForm (imageId, requestDelete = false) {
    ...
    const { data: formHtml } = await window.$http.get(
      `/images/edit/${imageId}`,
      params
    )
