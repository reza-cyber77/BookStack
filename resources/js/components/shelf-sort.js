...
export class ShelfSort extends Component {
  ...
  setupListeners () {
    this.elem.addEventListener('click', (event) => {
      const sortItemAction = event.target.closest(
        '.scroll-box-item button[data-action]'
      )
      if (sortItemAction) {
  ...
  filterBooksByName (filterVal) {
    ...
    if (!this.allBookList.style.height) {
      this.allBookList.style.height =
        this.allBookList.getBoundingClientRect().height + 'px'
    }
    ...
    for (const bookEl of books) {
      const show =
        !filterVal || bookEl.textContent.toLowerCase().includes(lowerFilter)
  ...
  onChange () {
    const shelfBookElems = Array.from(
      this.shelfBookList.querySelectorAll('[data-id]')
    )
    this.input.value = shelfBookElems
