...
export class MarkdownEditor extends Component {
  setup () {
    ...
    const settingInputs = settingContainer.querySelectorAll(
      'input[type="checkbox"]'
    )
  ...
  setupListeners () {
    ...
    this.elem.addEventListener('click', (event) => {
      ...
      const currentActiveSections = this.elem.querySelectorAll(
  ...
  handleDividerDrag () {
    this.divider.addEventListener('pointerdown', (event) => {
      ...
      const moveListener = (event) => {
        ...
        const xPct = Math.min(
          Math.max(20, Math.floor((xRel / wrapRect.width) * 100)),
          80
        )
