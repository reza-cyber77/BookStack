...
export class CodeEditor extends Component {
  ...
  setupListeners () {
    ...
    this.languageInput.addEventListener('input', (e) =>
      this.languageInputChange(this.languageInput.value)
    )
  ...
  sortLanguageList () {
    const sortedParents = this.languageButtons
      .sort((a, b) => {
  ...
  loadHistory () {
    this.history = JSON.parse(
      window.sessionStorage.getItem(this.historyKey) || '{}'
    )
    const historyKeys = Object.keys(this.history).reverse()
    this.historyDropDown.classList.toggle('hidden', historyKeys.length === 0)
    this.historyList.innerHTML = historyKeys
      .map((key) => {
