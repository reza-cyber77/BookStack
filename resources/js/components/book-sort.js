...
const moveActions = {
  ...
  next_chapter: {
    ...
    getNextChapter (elem, parent) {
      ...
      return topItems
        .slice(index + 1)
        .find((elem) => elem.dataset.type === 'chapter')
    }
  },
  prev_chapter: {
    active (elem, parent, book) {
...
export class BookSort extends Component {
  ...
  bookSelect (entityInfo) {
    const alreadyAdded =
      this.container.querySelector(
        `[data-type="book"][data-id="${entityInfo.id}"]`
      ) !== null
  ...
  setupBookSortable (bookContainer) {
    const sortElems = Array.from(
      bookContainer.querySelectorAll('.sort-list, .sortable-page-sublist')
    )
    ...
    for (const sortElem of sortElems) {
      Sortable.create(sortElem, {
        group: sortElem.classList.contains('sort-list')
          ? bookGroupConfig
          : chapterGroupConfig,
        animation: 150,
        fallbackOnBody: true,
  ...
  ensureNoNestedChapters () {
    const nestedChapters = this.container.querySelectorAll(
      '[data-type="chapter"] [data-type="chapter"]'
    )
    for (const chapter of nestedChapters) {
      const parentChapter = chapter.parentElement.closest(
        '[data-type="chapter"]'
      )
  ...
  buildEntityMap () {
    ...
    for (const list of lists) {
      ...
      const directChildren = Array.from(list.children).filter((elem) =>
        elem.matches('[data-type="page"], [data-type="chapter"]')
      )
