...
export class PageEditor extends Component {
  ...
  setupListeners () {
    ...
    // Changelog controls
    const updateChangelogDebounced = debounce(
      this.updateChangelogDisplay.bind(this),
      300,
      false
    )
  ...
  startAutoSave () {
    this.autoSave.interval = window.setInterval(
      this.runAutoSave.bind(this),
  ...
  runAutoSave () {
    ...
    const savedRecently =
      Date.now() - this.autoSave.last < this.autoSave.frequency / 2
  ...
  async saveDraft () {
    ...
    let didSave = false
    try {
      const resp = await window.$http.put(
        `/ajax/page/${this.pageId}/save-draft`,
        data
      )
      if (!this.isNewDraft) {
      ...
      this.draftNotifyChange(
        `${resp.data.message} ${Dates.utcTimeStampToLocalTime(
          resp.data.timestamp
        )}`
      )
      ...
      if (
        resp.data.warning &&
        !this.shownWarningsCache.has(resp.data.warning)
      ) {
  ...
  async changeEditor (event) {
    ...
    /** @var {ConfirmDialog} **/
    const dialog = window.$components.firstOnElement(
      this.switchDialogContainer,
      'confirm-dialog'
    )
    const [saved, confirmed] = await Promise.all([
      this.saveDraft(),
      dialog.show()
    ])
