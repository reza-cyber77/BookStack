...
export class Pointer extends Component {
  ...
  setupListeners () {
    ...
    DOM.onChildEvent(this.container, 'span.icon', 'click', (event, icon) => {
      ...
      icon.querySelector('[data-icon="include"]').style.display = !this
        .pointerModeLink
        ? 'inline'
        : 'none'
      icon.querySelector('[data-icon="link"]').style.display = this
        .pointerModeLink
        ? 'inline'
        : 'none'
  ...
  showPointerAtTarget (element, xPosition) {
    ...
    const xTarget = Math.min(
      Math.max(xPosition, targetBounds.left),
      targetBounds.right
    )
  ...
  updateForTarget (element) {
    let inputText = this.pointerModeLink
      ? window.baseUrl(`/link/${this.pageId}#${this.pointerSectionId}`)
      : `{{@${this.pageId}#${this.pointerSectionId}}}`
    if (this.pointerModeLink && !inputText.startsWith('http')) {
      inputText =
        window.location.protocol + '//' + window.location.host + inputText
    }
    ...
    if (editAnchor && element) {
      ...
      editAnchor.href = `${editHref}?content-id=${elementId}&content-text=${encodeURIComponent(
        queryContent
      )}`
    }
  }
