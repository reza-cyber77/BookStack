...
export class Shortcuts extends Component {
  ...
  runShortcut (id) {
    ...
    console.error(
      'Shortcut attempted to be ran for element type that does not have handling setup',
      el
    )
  ...
  showHintLabel (targetEl, key, wrapper) {
    ...
    label.style.insetInlineStart = `${
      targetBounds.x + targetBounds.width - (labelBounds.width + 6)
    }px`
    label.style.insetBlockStart = `${
      targetBounds.y + (targetBounds.height - labelBounds.height) / 2
    }px`
  }
