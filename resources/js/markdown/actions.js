...
export class Actions {
  ...
  insertLink () {
    ...
    this.editor.cm.setCursor(
      cursorPos.line,
      cursorPos.ch + newText.length + cursorPosDiff
    )
  }
  ...
  showLinkSelector () {
    ...
    selector.show((entity) => {
      const selectedText = this.editor.cm.getSelection() || entity.name
  ...
  insertDrawing (image, originalCursor) {
    ...
    this.editor.cm.setCursor(
      originalCursor.line,
      originalCursor.ch + newText.length
    )
  }
      ...
      (pngData) => {
        ...
        window.$http
          ...
          .then((resp) => {
            ...
            const newContent = this.editor.cm
              .getValue()
              .split('\n')
              .map((line) => {
  ...
  replaceLineStart (newStart) {
    ...
    if (lineStart === newStart) {
      ...
      this.editor.cm.replaceRange(
        ...
        { line: cursor.line, ch: lineLen }
      )
      this.editor.cm.setCursor({
        line: cursor.line,
        ch: cursor.ch - (newStart.length + 1)
      })
      return
    }
  ...
  wrapLine (start, end) {
    ...
    if (
      lineContent.indexOf(start) === 0 &&
      lineContent.slice(-end.length) === end
    ) {
      newLineContent = lineContent.slice(
        start.length,
        lineContent.length - end.length
      )
    } else {
  ...
  wrapSelection (start, end) {
    ...
    if (
      selection.indexOf(start) === 0 &&
      selection.slice(-end.length) === end
    ) {
      newSelection = selection.slice(
        start.length,
        selection.length - end.length
      )
  ...
  cycleCalloutTypeAtSelection () {
    ...
    const regex = new RegExp(
      `class="((${joint})\\s+callout|callout\\s+(${joint}))"`,
      'i'
    )
    ...
    if (format === formats[formats.length - 1]) {
      this.wrapLine(
        `<p class="callout ${formats[formats.length - 1]}">`,
        '</p>'
      )
    } else if (format === '') {
      ...
      const newContent = lineContent.replace(
        matches[0],
        matches[0].replace(format, newFormat)
      )
      this.editor.cm.replaceRange(
        newContent,
        contentRange.anchor,
  ...
  syncDisplayPosition () {
    ...
    const range = this.editor.cm.getRange(
      { line: 0, ch: null },
      { line: lineNum, ch: null }
    )
    const parser = new DOMParser()
    const doc = parser.parseFromString(
      this.editor.markdown.render(range),
      'text/html'
    )
  ...
  insertClipboardImages (images) {
    const cursorPos = this.editor.cm.coordsChar({
      left: event.pageX,
      top: event.pageY
    })
    this.editor.cm.setCursor(cursorPos)
