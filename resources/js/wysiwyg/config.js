...
const formats = {
  ...
  calloutsuccess: {
    block: 'p',
    exact: true,
    attributes: { class: 'callout success' }
  },
  calloutinfo: {
    block: 'p',
    exact: true,
    attributes: { class: 'callout info' }
  },
  calloutwarning: {
    block: 'p',
    exact: true,
    attributes: { class: 'callout warning' }
  },
  calloutdanger: {
    block: 'p',
    exact: true,
    attributes: { class: 'callout danger' }
  }
}
...
function fetchCustomHeadContent () {
  ...
  const startLineIndex = headContentLines.findIndex(
    (line) => line.trim() === '<!-- Start: custom user content -->'
  )
  const endLineIndex = headContentLines.findIndex(
    (line) => line.trim() === '<!-- End: custom user content -->'
  )
...
function getSetupCallback (options) {
  return function (editor) {
    editor.on(
      'ExecCommand change input NodeChange ObjectResized',
      editorChange
    )
...
export function build (options) {
  ...
  const version = document
    .querySelector('script[src*="/dist/app.js"]')
    .getAttribute('src')
    .split('?version=')[1]
  ...
  return {
    ...
    extended_valid_elements:
      'pre[*],svg[*],div[drawio-diagram],details[*],summary[*],div[*],li[class|checked|style]',
