    ...
    setTimeout(() => {
      ...
      uploadImageFile(imageFile, options.pageId)
        ...
        .catch((err) => {
          ...
          window.$events.emit(
            'error',
            options.translations.imageUploadErrorText
          )
          console.log(err)
        })
    }, 10)
...
async function uploadImageFile (file, pageId) {
  ...
  const resp = await window.$http.post(
    window.baseUrl('/images/gallery'),
    formData
  )
  return resp.data
}
...
function drop (editor, options, event) {
  ...
  const rng = tinymce.dom.RangeUtils.getCaretRangeFromPoint(
    event.clientX,
    event.clientY,
    editor.getDoc()
  )
  ...
  // Template insertion
  const templateId =
    event.dataTransfer && event.dataTransfer.getData('bookstack/template')
  if (templateId) {
    event.preventDefault()
